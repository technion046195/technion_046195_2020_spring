...
ונשערך כבנפדר את כל אחד משני הרכבים:

- מכיוון מאנו עוסקים בבעיות סיווגת אזי, $$p_Y$$ הינו פילוג דיסקרטי סופי, ולכן ניתן לשערך אותו בקלות על פי:

  $$
  p_Y\left(j\right)=\frac{1}{N}\sum_{i=1}^N I\left\lbrace y_i=j\right\rbrace
  $$

- את $$p_{\boldsymbol{X}\lvert Y}$$ ניתן לשערך על ידי חלוקה של המדגם על פי ערכי $$Y$$ של הדגמים, ושיערוך הפילוג המותנה בעבור כל חלק. **שימו לב**, מדובר בשיערוך של מספר פילוגים השווה למספר הערכים שהתוויות יכולות לקבל.
...
את הפילוגים החד מימדיים $$p_q\left(x_q\lvert y\right)$$ לומדים באופן בלתי תלוי בעבור כל ערך של $$Y$$ וכל אחד מ$$d$$ הרכיבים של $$\boldsymbol{X}$$.
...
כפי שניתן לראות, במקרה זה קו ההפרדה זז משמעותית לימין. ההסבר לזה היא העובדה היא שהפילוג הא-פריורי על התווית מקטין משמעותית את ההסתברות א-פריוירית שדגימה מסויימת הינה סרטנית, מה שמתבטא באופן דומה גם בסיכוי הפוסטריורי. לכן איזור גדול יותר ממופה עת לתווית של "לא סרטני".
