...
נתונה רשת קונבולוציה קטנה, הממירה תמונה בגודל 13×13 לווקטור מוצא בגודל 4×1. הרשת מורכבת מהפעולות הבאות:
 משכבות קונבולוציה עם 3 פילטרים (גרעינים) בגודל 4x4
  , max-pooling, Relu כפונקציית אקטיבציה ו Fully-connected (FC) בסוף. ברשת זאת אין שימוש בפרמטר חופשי -bias .
![png](Q1.png)
 	
 א. כמה משקולות יש בשכבת הקונבולוציה שעוברות למידה?
 
כמה פעולות אקטיבציה (Relu) במעבר קדמי אחד (forward pass)?
...
ב. בוצעים 75 פעולות Relu. פעולת האקטיבציה מתבצעת לאחר שכבת הmax pooling. הפעולה מתבצעת על כל פיקסל וישנם סה"כ 3⋅5⋅5=75.
...
1. לאילו מפונקציות האקטיבציה שלמדנו בקורס (sigmid, tahn ו-ReLU) ההנחה Ex_i=0 שביצענו בסעיף הקודם היא עדיין הנחה "סבירה"?

תזכורת:

![png](Q3_3.png)

2. 	כעת לא נניח דבר על התוחלת של  ִִ$$x_i$$. כיצד ישתנה הביטוי של השונות של  $$v$$ מסעיף א.3?
...
3.	תחת ההנחה כי התוחלות של המשקולות ושל הכניסה הן 0, הנוסחה שהוכחנו בסעיף הקודם מצטמצמת ל-
$$
...
$$
Ex_1^2=E((max ((0,y_1 ) )^2 ) = \int_{-\infty}^{-\infty} (max x(0,y_1 ) )^2 f_Y (y)dy
=\int_{-\infty}^{-\infty} y_1^2 f_Y (y)dy = \frac{1}{2} \int_{-\infty}^{-\infty} y_1^2 f_Y (y)dy = 1/2 Ey_1^2= \frac{1}{2} Var(y_1 )
$$
