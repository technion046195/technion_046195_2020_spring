...
רשתות ניורונים מלאכותיות (Artificial Neural Networks (ANN)) הינם מודלים מתמטיים אשר בנויים בהשראת רשתות נוירונים ביולוגיותץ בדומה לרשות הביולוגיות, הם בנויות כאוסף של יחידות המכונות לרוב נוירונים מלאכותיים, אשר מבצעות כל אחת פעולה מתימטית פשוטה. כאשר משלבים הרבה נויורונים כאלה יחד (מיפוי המוצאים של נויורונים מסויימות לכניסות של נוירונים איחרים) ניתן למדל פונקציות מורכבות.

![ANN](../../media/diagrams/networks/ann.png)

בחירה נפוצה לפעולה המתימטית אותה מבצעים הנוירונים הינה של פונקציה לא נינארית סקלרית הפועלת על קומבינציה לינארית בתוספת קבוע (bias) של הכניסות שלה:
...
הנוירונים אשר אינם מחוברים למוצא הרשת (אינם נמצאים בסוף הרשת) מוכנים **יחידות נסתרות** (**hidden units**). רשתות אשר מכילות מסלולים מהכניסה למוצא אשר עוברים דרך יותר מיחידה נסתרת אחת מכונות **רשתות עמוקות** (**Deep Neural Networks (DNN)**)

הצורה שבה הנוירונים מחוברים בתור הרשת נקראת הארכיטקטורה של הרשת. אנו מבדילים בין שני סוגי ארכיטקטורות:
...
נוכל כמובן לרשום זאת גם כגרף אחד כאשר אנו מאחדים את כל החישובים הזהים ובכך מייעלים את החישוב:

![Backwoard graph](../../media/diagrams/networks/back.png)

נשים לב כי למעשה לגרף זה צורה דומה לגרף של הפונקצייה המקורית, אם כי בכיוון הפוך. באופן כללי ניתן להראות כי ניתן להפוך כל גרף של פונקציה על מנת לקבל את הגרף של הנגזרת שלה באופן הבא:
