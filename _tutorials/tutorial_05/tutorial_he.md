...
לדוגמא: מתן תחזית לערך מניה מסויימץ על סמך נתוני הבורסה.
...
- סוגיה יסודית בבחירת סדר המודל הינה הניגוד הבא:
  ...
  - מודל מסובך מדי (בעל סדר גבוהה) הוא בעל מספר גדול של דרגות חופש, ולפיכך עלול לדרוש מספר רק של דוגמאות על מנת לבצע הכללה סבירה.
...
- $$E_{\text{app}}\left(F\right)$$ היא שגיאת הקירוב (approximation error) הנקאת גם ההטיה (bias). היא מציינת את הסיכון המינמאלי שחזאי **כלשהוא** מתוך המודל $$F$$ יכול להשיג. גודל זה הינו דטרמיניסטי ואינו תלוי בסדרת הלימוד.
...
2. סט אימות (validataion set) - סט דוגמאות מתוייג $$D=\left\lbrace x_i,y_i\right\rbrace_{i=1}^n$$ שבאמצעותו נעריך את טיבם של המודלים על מנת לבחור בניהם.
...
עבור K=1, מספר השיאות הוא 3, ולכן שגיאת ה-CV הינה: $$\hat{L}_n^1=\frac{3}{7}$$
עבור K=3, מספר השיאות הוא 4, ולכן שגיאת ה-CV הינה: $$\hat{L}_n^1=\frac{3}{7}$$
...
על פי הגרף, הK האופטימאלי הינו $$K=19$$ אשר נותן סיכון משעורך על סט הבוחן של $$0.097$$.
...
על פי הגרף, הK האופטימאלי הינו $$K=22$$ אשר נותן סיכון משעורך על סט הבוחן של $$0.087$$.
